...
// Create a playlist and make sure it gets added to the repository
  it("only add playlists", () => {
    var options:any = {method: 'POST', uri: (base_url + "library"), body:{creatorId: 1, playlistName: "Dance Music", songs: []}, json: true};
    return request(options)
             .then(body =>
               //  expect(body).toEqual({success: "Playlist successfully created."})
               expect(body).toEqual({id:0, creatorId: 1, playlistName: "Dance Music", songs:[]})
             ).catch(e =>
                expect(e).toEqual(null)
             )
  });
  ...
  it("only delete playlist", () => {
    var options:any = {method: 'DELETE', uri: (base_url + "library/delete/:Dance Music"), body:{id:1, creatorId: 1, playlistName: "Dance Music", songs: []}, json: true};
    return request(options)
             .then(body =>
                expect(body).toEqual({success: "Playlist successfully deleted."})
             ).catch(e =>
                expect(e).toEqual(null)
             )
  });
...
// Add a song to the repo
 it("only add songs", () => {
   var options:any = {method: 'POST', uri: (base_url + "song/create"), body:{id:21, songName: "Yellow Submarine", artistName: "Beatles", albumId: 1, length: 120}, json: true};
   return request(options)
            .then(body =>
...
it("only add playlists", () => {
  var options:any = {method: 'POST', uri: (base_url + "library"), body:{creatorId: 1, playlistName: "Fun Tunes", songs: []}, json: true};
  return request(options)
           .then(body =>
              expect(body).toEqual({id:1, creatorId: 1, playlistName: "Fun Tunes", songs: []})
           ).catch(e =>
              expect(e).toEqual(null)
           )
});
...
it("do not add playlists with duplicate names", () => {
  var opt1:any = {method: 'POST', uri: (base_url + "library"), body:{creatorId: 1, playlistName: "Jazz",songs: []}, json: true};
  var opt2:any = {method: 'POST', uri: (base_url + "library"), body:{creatorId: 2, playlistName: "Jazz",songs: []}, json: true};
...
it("search for a playlist", () => {
   var playlist:any = {method: 'POST', uri: (base_url + "library"), body:{creatorId: 0, playlistName: "Hippy",songs: []}, json: true};
   request(playlist);
...
it("search for a song and add it to a playlist", () => {
   var options:any = {method: 'PUT', uri: (base_url + 'search/add-song-1/to-playlist-Fun Tunes'), body:{}, json: true};
   return request(options)
            .then(body =>
              expect(body).toEqual([{ id: 1, songName: 'Welcome to New York', artistName: 'Taylor Swift', albumId: 1, length: 212 }])
            ).catch(e =>
               expect(e).toEqual(null)
            )
 });
...
it("remove a song from a playlist", () => {
   var options:any = {method: 'PUT', uri: (base_url + `library/remove-song-1/from-playlist-1`), body:{}, json: true};
   return request(options)
            .then(body =>
              expect(body).toEqual([])
            ).catch(e =>
